...
# try workig with the log of the variable
...
So, in the case of mutiple boxplots, we can get the same type of information:
...
2. use the split command to partition the data frame into 4 groups which correspond to each of the four seasons. 
...
4. grid - This is a low level library that all the other graphic packages use
...
- Graphs are usually stsandard types but combined in interesting ways
- Comparisons across panels and facets
- Axes, legends, titles, colors, groups are ususally present
...
- Flexibly explore the data using a number of vizualizations (or combinations thereof)
...
ggplot2 is part of the ''tidyverse'' which is a collection of R packages that share common philosphies to work well together
...
- Convenient wrapper for qucikly creating a number of different types of plots
...
```{r}
qplot(mpg,wt,data=mtcars,main="MPG vs wt",facets=cyl ~ .)

```

Specifying a "geometry" is a powerful part og ggplot
...
- lines and variations (dashed, segements, etc)
- bars, histograms
- text labels
- points
...
What are the categories/factors in this data ? What are the continuous quantitites ?
...
```{r}
ggplot(mtcars) + geom_density(aes(x=mpg,fill=cyl)) + theme_bw()
```
...
Facets allow us to display data within panels to facilitate easy comparisons In lattice graphics this is called panelling or conditioning
